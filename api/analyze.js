...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!body || !body.images || !body.images.length) {
      return res.status(400).json({ error: "No images in request body" });
    }
    ...
    body.images.forEach((img) => {
      content.push({ type: "text", text: "--- " + (labelMap[img.type] || img.type) + " ---" });
      content.push({
        type: "image",
        source: {
          type: "base64",
    ...
    if (!response.ok) {
      ...
      return res.status(502).json({ error: "Anthropic API error " + response.status, details: errText });
    }
    ...
    console.log("Analysis complete:", result.findings ? result.findings.length : 0, "findings");
